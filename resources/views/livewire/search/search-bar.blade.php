<div class="w-100 position-relative">
    ...
    <div class="d-flex align-items-center position-relative">
        <input wire:model.live.debounce.300ms="query" wire:keydown.enter="search" type="text"
            placeholder="Cari vendor atau layanan..." class="form-control rounded-pill py-2 ps-4 pe-5 shadow-sm border-0"
            style="width: 100%;">
        <button wire:click="search" class="btn position-absolute end-0 me-2 text-primary bg-transparent border-0"
                                ...
                                <a href="{{ route('vendor.show', $vendor->id) }}"
                                    ...
                                    <div class="d-flex align-items-center">
                                        <img src="{{ $vendor->profile_picture ? asset('storage/' . $vendor->profile_picture) : 'https://ui-avatars.com/api/?name=' . urlencode($vendor->business_name) . '&background=92B6B1&color=white&size=120' }}"
                                            alt="{{ $vendor->business_name }}" class="rounded-circle me-2"
                                    ...
                                    <div class="d-flex align-items-center">
                                        @if ($service->image_url)
                                            <img src="{{ asset('storage/' . $service->image_url) }}"
                                                alt="{{ $service->name }}" class="rounded me-2"
                                                style="width: 32px; height: 32px; object-fit: cover;">
                                        @else
