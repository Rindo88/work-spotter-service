...
class ScheduleForm extends Component
{
    ...
    public function updatedSchedules($value, $key)
    {
        ...
        if (count($keyParts) >= 3) {
            ...
            // Validasi waktu
            if (in_array($field, ['open', 'close']) && $value) {
    ...
    private function validateTime($day, $field, $time)
    {
        ...
        // Validasi open time harus sebelum close time
        if ($field === 'close' && !$this->schedules[$day]['closed']) {
            $openTime = strtotime($this->schedules[$day]['open']);
        ...
        foreach ($this->schedules as $day => $schedule) {
            if (!$schedule['closed']) {
                if (empty($schedule['open']) || empty($schedule['close'])) {
                    $this->addError("schedules.{$day}.open", "Waktu operasional hari {$day} harus diisi");
                    return false;
                }
                
                $openTime = strtotime($schedule['open']);
                $closeTime = strtotime($schedule['close']);
                
                if ($closeTime <= $openTime) {
                    $this->addError("schedules.{$day}.close", "Waktu tutup harus setelah waktu buka pada hari {$day}");
                    return false;
                }
